...
function MVPNavigation() {
  const navigate = useNavigate();
  const location = useLocation();

  const pages = [
    { path: '/mvp/login', label: '작업자 로그인', icon: '👤' },
    { path: '/mvp/dashboard', label: '대시보드', icon: '🏠' },
    { path: '/mvp/checklist', label: '체크리스트', icon: '📋' },
    { path: '/mvp/result', label: '판정 결과', icon: '✅' },
  ...
  return (
    <div style={{
      ...
      <div style={{
        gridColumn: '1 / -1',
        height: '4px',
        background: location.pathname !== '/mvp/login' ? '#52c41a' : '#ff4d4f',
        borderRadius: '2px',
        marginBottom: '4px',
        boxShadow: `0 0 6px ${location.pathname !== '/mvp/login' ? '#52c41a' : '#ff4d4f'}`
      }} />
